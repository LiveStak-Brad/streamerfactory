-- Seed Content Creation Mastery lesson so /streameru/seasonal-content-without-gimmicks resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Content Creation lesson seed (seasonal-content-without-gimmicks): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'seasonal-content-without-gimmicks',
    'Build a 90-day seasonal content calendar with three peaks — holidays and personal seasons without forced trends.',
    $cc248$
## Introduction
