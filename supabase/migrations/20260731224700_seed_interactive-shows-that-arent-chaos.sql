-- Seed Content Creation Mastery lesson so /streameru/interactive-shows-that-arent-chaos resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Content Creation lesson seed (interactive-shows-that-arent-chaos): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'Interactive Shows That Aren''t Chaos',
    'interactive-shows-that-arent-chaos',
    'Build one interactive show format with rules, roles, and kill-switches that stays hostable when chat spikes.',
    $cc247$
## Introduction
