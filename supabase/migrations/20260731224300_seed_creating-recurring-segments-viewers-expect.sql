-- Seed Content Creation Mastery lesson so /streameru/creating-recurring-segments-viewers-expect resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Content Creation lesson seed (creating-recurring-segments-viewers-expect): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'creating-recurring-segments-viewers-expect',
    'Launch two named recurring segments with rules, length, and payoff so return viewers have a reason to show up.',
    $cc243$
## Introduction
