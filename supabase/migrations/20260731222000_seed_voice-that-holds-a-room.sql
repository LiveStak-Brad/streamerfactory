-- Seed Presence Mastery lesson so /streameru/voice-that-holds-a-room resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (voice-that-holds-a-room): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'voice-that-holds-a-room',
    'Apply a five-tool vocal warm-up and vary pace and energy across a LIVE segment.',
    $pr2000$
## Introduction
