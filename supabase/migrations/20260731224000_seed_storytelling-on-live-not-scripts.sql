-- Seed Presence Mastery lesson so /streameru/storytelling-on-live-not-scripts resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (storytelling-on-live-not-scripts): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'storytelling-on-live-not-scripts',
    'Deliver micro-stories with setup, turn, and payoff — memory without essays on camera.',
    $pr4000$
## Introduction
