-- Seed Presence Mastery lesson so /streameru/humor-warmth-and-authenticity resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (humor-warmth-and-authenticity): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'Humor, Warmth, and Authenticity Without Forcing It',
    'humor-warmth-and-authenticity',
    'Practice three authentic personality levers without forced bits.',
    $pr7000$
## Introduction

This is a Presence Mastery lesson. Complete the full StreamerU lesson body, downloads, quiz, and LIVE Mission on the site.
$pr7000$,
    null, cat_content, seed_author, 'published', false, now(), 'presence', 'intermediate'
  )
