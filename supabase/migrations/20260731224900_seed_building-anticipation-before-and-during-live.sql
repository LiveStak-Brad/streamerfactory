-- Seed Content Creation Mastery lesson so /streameru/building-anticipation-before-and-during-live resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Content Creation lesson seed (building-anticipation-before-and-during-live): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'building-anticipation-before-and-during-live',
    'Install an anticipation system across pre-LIVE, mid-LIVE, and end-LIVE — teasers and promises you actually keep.',
    $cc249$
## Introduction
