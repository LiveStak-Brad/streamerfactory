-- Extension-based TikTok Creator Network imports (audit batches + stats + live snapshots).
-- Does not replace creator_performance_stats / seed leaderboard in this phase.
...
-- ---------------------------------------------------------------------------
-- creator_network_live_snapshots (LIVE now page syncs)
-- ---------------------------------------------------------------------------
...
-- Stats: members read own row (includes financial columns for self only)
...
-- Live snapshots: network members can read (no money fields on this table)
...
comment on table public.creator_network_import_batches is
  'Audit trail for Chrome extension TikTok Creator Network page syncs (staff only).';

comment on table public.creator_network_member_stats is
  'Imported Creator Network stats and relationship rows; staff see all, members see own row via RLS.';

comment on table public.creator_network_live_snapshots is
  'Latest LIVE now snapshot from TikTok Backstage; safe for member-facing Live Now UI.';
