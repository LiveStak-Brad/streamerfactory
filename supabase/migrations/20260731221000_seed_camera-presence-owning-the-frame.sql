-- Seed Presence Mastery lesson so /streameru/camera-presence-owning-the-frame resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (camera-presence-owning-the-frame): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'camera-presence-owning-the-frame',
    'Train camera authority with a 10-minute presence drill and honest self-review notes.',
    $pr1000$
## Introduction
