-- Seed Presence Mastery lesson so /streameru/presence-capstone-signature-20-minute-live resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (presence-capstone-signature-20-minute-live): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'presence-capstone-signature-20-minute-live',
    'Deliver a signature 20-minute LIVE with a reviewable presence evidence package.',
    $pr0000$
## Introduction
