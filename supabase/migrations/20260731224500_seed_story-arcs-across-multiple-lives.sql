-- Seed Content Creation Mastery lesson so /streameru/story-arcs-across-multiple-lives resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Content Creation lesson seed (story-arcs-across-multiple-lives): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'story-arcs-across-multiple-lives',
    'Design a 3–7 session story arc with checkpoints and a finale — appointment viewing without scripted drama.',
    $cc245$
## Introduction
