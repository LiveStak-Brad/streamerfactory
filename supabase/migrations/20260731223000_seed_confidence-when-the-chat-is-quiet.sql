-- Seed Presence Mastery lesson so /streameru/confidence-when-the-chat-is-quiet resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (confidence-when-the-chat-is-quiet): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'confidence-when-the-chat-is-quiet',
    'Use a quiet-chat protocol to complete a full segment without chat dopamine.',
    $pr3000$
## Introduction
