-- Allow Creator Wellness & Longevity Mastery lessons to use training_track = 'wellness'.
...
alter table public.resource_posts add constraint resource_posts_training_track_check check (
  training_track = any (array[
    ...
    'tts'::text, 'wellness'::text
  ])
);
comment on column public.resource_posts.training_track is
  'Program track: beginner, battles, monetization, rules, content, presence, creation, growth, community, professional, production, battle, music, gaming, multiguest, aicreator, selling, tts, wellness.';
