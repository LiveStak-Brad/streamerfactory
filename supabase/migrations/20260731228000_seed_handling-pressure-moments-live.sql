-- Seed Presence Mastery lesson so /streameru/handling-pressure-moments-live resolves.
...
begin
  ...
  if seed_author is null then
    raise notice 'Skipping Presence lesson seed (handling-pressure-moments-live): no auth.users row.';
    return;
  end if;
  ...
  insert into public.resource_posts (
    ...
    'handling-pressure-moments-live',
    'Execute recovery scripts for trolls, tech fails, gifts, and sudden crowds.',
    $pr8000$
## Introduction
