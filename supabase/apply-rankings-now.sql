...
-- Helpers (fixes: function public.can_schedule_battles() does not exist)
...
create index if not exists creator_performance_stats_profile_id_idx on public.creator_performance_stats (profile_id);
create index if not exists creator_performance_stats_period_idx on public.creator_performance_stats (period_start, period_end);
create index if not exists creator_rankings_period_idx on public.creator_rankings (ranking_period, period_start);
create index if not exists creator_rankings_position_idx on public.creator_rankings (ranking_period, period_start, rank_position);
...
for each row execute function public.set_creator_performance_stats_updated_at ();
...
for select to authenticated using (public.can_schedule_battles ());
...
create policy "creator_rankings_select_network" on public.creator_rankings
for select to authenticated using (public.can_schedule_battles ());

drop policy if exists "creator_rankings_staff_write" on public.creator_rankings;
create policy "creator_rankings_staff_write" on public.creator_rankings
for all to authenticated using (public.is_staff ()) with check (public.is_staff ());
...
-- Public leaderboard (anon can read via RPC on /rankings)
