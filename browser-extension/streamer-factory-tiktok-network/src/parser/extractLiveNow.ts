...
function liveCardElements(doc: Document): Element[] {
  const selectors = [
    "table tbody tr",
    '[role="row"]',
...
function parseLiveCard(el: Element): ParsedLiveRow | null {
  ...
  let liveStartedText: string | undefined;
  ...
  const liveBadgeDetected =
    /\blive\b/i.test(text) || Boolean(el.querySelector('[class*="live"], [class*="Live"], [aria-label*="live" i]'));
