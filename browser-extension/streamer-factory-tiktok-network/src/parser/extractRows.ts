...
function parseStatsRow(row: Element): ParsedCreatorRow | null {
  const cells = cellTexts(row);
  const cellEls = tableCells(row);
  if (cells.length === 0) return null;
  ...
  if (coins === undefined) coins = firstCompactNumber(joined);
  if (diamonds === undefined) diamonds = coins;
